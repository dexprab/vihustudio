...
function makeHandler(deps: Deps) {
  ...
  if (payload.action === 'imagine') {
    ...
    const ires = await boundedFetch(doFetch, PROVIDER_IMAGE_URL, {
      ...
      body: JSON.stringify({ model: imageModel, prompt, n, size: '1024x1024' }),
    }, 110000);
    ...
    return json({ ok: true, images, model: imageModel, build: BUILD });
  }
...
export { makeHandler, handler, BUILD, DEFAULT_MODEL, DEFAULT_IMAGE_MODEL, MAX_MESSAGES, MAX_MESSAGE_CHARS, MAX_IMAGES, MAX_IMAGE_B64_CHARS, IMAGE_MIMES };
