...
| Shown in Creator | `js/objectStrip.js` lists every rendered Layer Pack entry as a Scene Object card, exactly once, sourced purely from the render tree (`SlideRenderer.getSceneElements()` — Creator Reconciliation Sprint; Object Strip no longer separately queries `SceneEngine` at all). Badge reads 🟢 "You can edit" or 🔒 "Part of the world" from the object's own `editable`/`owner` fields. |
| Editable by creator? | **Depends on real Builder-authored data, as of the Creator Reconciliation Sprint** — no longer a blanket `locked:true`. A Scene Layer converged via World Builder's `convergeSceneLayer` carries real `moveable`/`editable` values sourced from the Builder Author's own `layer.permissions` (`js/projectModel.js`); Context Panel reads the selected object's `editable` flag directly (`js/contextPanel.js`'s `_findSceneObject`/`_renderWorldObjectDisclosure`) rather than deciding by type. Either way (editable or not), a World-owned object never opens Card Designer's generic decoration/text/sticker section — that section reads `slide.metadata.elementOverrides`, not the object's own id, so it would silently target nothing; instead a plain disclosure shows, worded differently for `editable:true` ("part of the World, but you may adjust it — not available in Creator yet") vs `editable:false` ("part of the World"). A hand-authored, pre-this-sprint Layer Pack entry (Museum Gallery's legacy shape) carries neither field and resolves `false`/locked exactly as before — zero behaviour change for existing themes. |
...
| Reflected in | Canvas (always, if the theme has one), Object Strip, Publish. Card Designer's own bound controls still never touch a Layer Pack entry directly — building a real generic editor for Builder-owned content is a disclosed future phase, not implemented here. |
