...
| `moveable` | Optional, default `true` | **Honour World-Owned Object Commitments sprint** (superseding the Creator Reconciliation Sprint's own disclosed "not yet acted on" note — this is now real). Mirrors `visible`'s own default-open convention (absent means Builder's own `layer.permissions` was never authored, e.g. every hand-authored `layer-packs/*.json` entry before this sprint — resolved `false` at Studio's own render step, not here, since a hand-authored entry never sets this key at all; see `renderer/slideRenderer.js`'s `_pushLayerObject`, which now derives the object's `locked` flag directly from this field). For a Scene-authored Layer converged via World Builder's `convergeSceneLayer`, this is a real, per-object value sourced from the Builder Author's own permission authoring (`js/projectModel.js`'s `layer.permissions.moveable`) — whether a Story Author may reposition this object in Creator. `true` engages the exact same drag machinery Cover/Hook/End blueprint elements already use, persisting the new position into the same `slide.metadata.elementOverrides` bag every other per-object override already lives in (keyed by this Layer's own compiled id). |
| `editable` | Optional, default `true` | Same convention and source as `moveable`, for the Content/Modify capability instead of Move. `true` now (Honour World-Owned Object Commitments sprint) surfaces one real, kind-specific in-place edit control in Creator's Context Panel — a colour picker for a `fill`/`shape` decoration's own colour, a Replace Image button for an `image` decoration, or a Words textarea for a `text` Layer (a bare glyph decoration/sticker has no such control — recolouring/replacing a glyph isn't offered, a disclosed simplification) — writing through `SceneEngine.setContentOverride()` into the same `elementOverrides` bag `moveable` uses. Never opens Card Designer's generic, id-blind section for a World-owned object either way, and never offers deletion/removal of a World-owned object regardless of this flag. |
| `decorationSlot` | Optional, default `false` | **Honour World-Owned Object Commitments sprint.** Mirrors World Builder's 4th permission-block checkbox, "Let the Story Author add their own decorations here too" (`layer.decorationSlot`, a boolean sibling of `layer.permissions`, not inside it) — previously authored in Builder but never copied into the compiled package at all (dead data on both sides until this sprint). `true` shows a "+ Add your own decoration here" button in Creator's Context Panel when this object is selected, which opens the existing Sticker Studio flow (pick-from-library → place-on-canvas → `slide.metadata.stickers[]`) seeded to place the very next sticker near this object's own position — no new persistence mechanism, since a Story-Author-added decoration was always meant to be an ordinary, fully Story-owned sticker, not a second kind of World content. |
