...
| `moveable` | Optional, default `true` | **Creator Reconciliation Sprint.** Mirrors `visible`'s own default-open convention (absent means Builder's own `layer.permissions` was never authored, e.g. every hand-authored `layer-packs/*.json` entry before this sprint — resolved `false` at Studio's own render step, not here, since a hand-authored entry never sets this key at all; see `renderer/slideRenderer.js`'s `_pushLayerObject`). For a Scene-authored Layer converged via World Builder's `convergeSceneLayer`, this is a real, per-object value sourced from the Builder Author's own permission authoring (`js/projectModel.js`'s `layer.permissions.moveable`) — whether a Story Author may reposition this object in Creator. Read by Creator's render tree, not yet acted on (no drag-persistence exists for a World-owned object yet — a disclosed, deliberately deferred follow-up). |
| `editable` | Optional, default `true` | Same convention and source as `moveable`, for the Content/Modify capability instead of Move — whether a Story Author may change this object's content/styling in Creator. Read by Creator's Context Panel to decide whether a selected World-owned object shows a plain disclosure or (a future phase) a real editor; never opens Card Designer's generic, id-blind section for a World-owned object either way. |
