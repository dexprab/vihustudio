# Hero Page Bible

This document is the top-level index and vision statement for the
VihuPlanet Hero Page. It is a skeleton — each section will be filled in as
the corresponding contracts are drafted, reviewed, and locked.

---

## Purpose

_Placeholder — why the Hero Page exists and what role it plays before a
child enters VihuPlanet._

---

## Vision

_Placeholder — the long-term aspiration for the Hero Page experience._

---

## Design Philosophy

_Placeholder — the guiding design beliefs behind the Hero Page._

---

## Emotional Goals

_Placeholder — the feelings the Hero Page must create in a child. See
`HERO-EMOTIONS.md` for detail._

---

## User Journey

_Placeholder — the step-by-step path a child takes through the Hero Page._

---

## Core Contracts

_Placeholder — index of the numbered contract documents and their current
status._

- [Contract 01 — Purpose](01-CONTRACT-Purpose.md) — 🟢 LOCKED
- [Contract 02 — Exploration](02-CONTRACT-Exploration.md) — 🟢 LOCKED
- [Contract 03 — Storyteller](03-CONTRACT-Storyteller.md) — 🟢 LOCKED
- [Contract 04 — Universe Relationships](04-CONTRACT-Universe-Relationships.md) — 🟡 DRAFT
- [Contract 05 — Home Planet](05-CONTRACT-Home-Planet.md) — 🟡 DRAFT
- [Contract 06 — Dreaming Planet](06-CONTRACT-Dreaming-Planet.md) — 🟡 DRAFT
- [Contract 07 — Authentication](07-CONTRACT-Authentication.md) — 🟡 DRAFT
- [Contract 08 — User Switch](08-CONTRACT-User-Switch.md) — 🟡 DRAFT
- [Contract 09 — Browser Memory](09-CONTRACT-Browser-Memory.md) — 🟡 DRAFT
- [Contract 10 — World Objects](10-CONTRACT-World-Objects.md) — 🟡 DRAFT
- [Contract 11 — Identity](11-CONTRACT-Identity.md) — 🟡 DRAFT
- [Contract 12 — Companion](12-CONTRACT-Companion.md) — 🟡 DRAFT

---

## World Principles

_Placeholder — rules governing the Hero Page's world and setting._

---

## Engineering Principles

_Placeholder — technical constraints and principles for implementation._

---

## Non-Negotiables

_Placeholder — see `HERO-NON-NEGOTIABLES.md` for the full list._

---

## Version History

_Placeholder — summary of major revisions to this document._
