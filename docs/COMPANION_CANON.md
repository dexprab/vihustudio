...
(Lumo's pre-canon pose set also included `talk`, retired by this
freeze — see `docs/COMPANION_ENGINE.md`'s changelog for the exact
migration.)
...
Registry (`assets/companions/registry.json`) — no new registry
mechanism, no hardcoded paths:

```
assets/companions/
  story-egg/
    hero.png · idle.png · curious.png · thinking.png ·
    excited.png · sleep.png · hatching.png
    companion.json
    animations.json
  lumo/
    hero.png · idle.png · wave.png · curious.png ·
    think.png · celebrate.png · sleep.png
    companion.json
    personality.json
    animations.json
```

`assets/companions/story-egg/companion.json` is registered as a
canonical platform entity as of this sprint; its 7 pose PNGs are the
one piece of this sprint genuinely still pending — see
`docs/COMPANION_ENGINE.md`'s "Asset Status" section for exactly what's
disclosed as outstanding and why nothing was fabricated in their
place.
