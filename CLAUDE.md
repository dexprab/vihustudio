...
- WYSIWYE Rendering (Sprint 9.0.2 — `SlideRenderer.init(canvas, opts)` accepts `{dpr}`, defaults to `window.devicePixelRatio`; editor + Publish Read canvases render at native pixel density; export paths force `{dpr:1}` for predictable output; hit-testing reads canonical size via `SlideRenderer.getCanvasSize()`)
