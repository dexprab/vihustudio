...
- **THE THING SERVED IS THE THING UNDER TEST — AND THE LAB WAS NOT
  STAMPING WHAT IT SERVED.** Reported by the product owner one build
  later: *"i pressed join, nothing joined."* His screenshot showed the
  NEW help text under the tools and the OLD button behaviour — which is
  the signature of `shape.html` arriving fresh while the browser kept
  serving the `labShape.js` it already held. The Studio pins every
  script with `?v=<build>` for exactly this; the four Lab pages loaded
  every script bare. **Every Lab script tag now carries `?lab=<stamp>`**,
  a hash of the CONTENTS of everything the four pages load — the Lab's
  own modules and the production files it reuses alike — generated by
  `tools/ether-mystery-lab/stamp.js`. The build stamp (`?v=0769`) is
  untouched: it names what shipped to a child, and the Lab ships
  nothing to a child. **The suite asks the generator itself** whether
  anything drifted (`S3c`), so a Lab change that forgot to restamp
  cannot pass — proved by editing a module without restamping and by
...
- **THE TRANSPORTS ARE THE EXISTING ONES, WIDENED, AND THE ENDPOINT IS
  `LAB3`.** `LabConnection.images()` asks for three candidates in
  ...
  provider's images route with the typed key and the Image model field's
  own model, and endpoint mode asks `lab-generate`'s ONE image route —
  `imagine`, which now takes an optional `quality` and `format` from two
  short lists — for one candidate per request. (The sprint first shipped
  a separate `image` action; the parallel prompt → visual → understanding
  sprint had landed `imagine` and `understand` on `main` at the same
  time, and merging the two kept ONE image route rather than two.) The
  function also relays a `generate` message whose content is PARTS —
  text plus ONE bounded image data URL, refused by name otherwise —
