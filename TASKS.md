...
3. Wire up or implement behaviors for `+ Cover` and `+ CTA` buttons (currently present in HTML but have no dedicated handlers).
...
8. Add a documented CHANGELOG process and release notes.
...
- The `+ Cover` and `+ CTA` buttons do nothing (no dedicated event handlers in `js/app.js`).
...
2. Add undo/redo support for page operations using a simple command stack.
3. Add handlers for `+ Cover` and `+ CTA` that insert special slide types.
