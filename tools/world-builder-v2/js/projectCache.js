...
(function () {
    ...
    // of Worlds).
    //
    // IN-FLIGHT GUARD — a real, confirmed bug found chasing a live
    // "Cloud backup failed" / Postgres 57014 statement-timeout report on
    // builder_projects: with four independent triggers (boot, `online`,
    // the 60s interval, and putLocal()/markCloudSynced()'s own 2s
    // _scheduleDrainSoon debounce) and no coordination between them, two
    // of these could legitimately fire close together and both start a
    // drain cycle — each reading the SAME "pending" record (the first
    // hasn't flipped its status to 'done' yet, since its own push() is
    // still in flight) and both calling _attemptSync(id) concurrently.
    // That's two separate DB transactions racing to UPDATE the exact
    // same builder_projects row; the second blocks on the first's row
    // lock until it commits, and any real-world hiccup holding the first
    // transaction open (a slow round trip, an interrupted connection)
    // turns that ordinary sub-millisecond wait into a genuine, repeating
    // statement-timeout. _drainInFlightPromise makes every one of the
    // four triggers share the SAME in-flight drain instead of starting a
    // second, overlapping one — set synchronously before any await, so
    // even two calls in the same tick correctly de-duplicate. A record
    // that becomes due WHILE a drain is already running simply waits for
    // the next trigger (worst case ~60s later, the background interval)
    // — never a durability risk, since the record is already safely
    // durable in IndexedDB regardless of when its cloud push happens.
