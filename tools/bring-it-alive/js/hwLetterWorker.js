 ...
 *
 * hwLetter.js is DOM-free, so it loads here verbatim (window := the
 * worker global; it no longer needs segment.js — the letter reader
 * owns its own colour-distance ink question). The whole read is
 * time-boxed by hwLetter's own BUDGET_MS; an overrun answers 'nothing'
 * and the frame is skipped — refusing a live frame is invisible (the
 * light just stays red; the next frame is half a second away).
 *
...
self.window = self;   // the module attaches its exports to `window`
importScripts('hwLetter.js');
...
self.onmessage = (e) => {
  ...
  try {
    ...
    const out = HWLetter.read(photo, {
      ...
      ch: m.ch || null            // the tapped identity: the sliver rule's class
    });
    // The frame analysed may be a centre crop of the sensor (the small
    // letter window); the live loop's steadiness bars are about a hand
    // in the WORLD, so the sensor's own width travels with the verdict.
    if (m.sensorW) out.facts.sensorW = m.sensorW;
    let skipped = null;
